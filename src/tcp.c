...
process_tcp(u_char * data, int skblen)
{
  ...
  //一个方向上的tcp流,tcp分为两个方向上的,一个是客户端到服务器端，一个是服务器端到客户

  ugly_iphdr = this_iphdr;
  iplen = ntohs(this_iphdr->ip_len);
  if ((unsigned)iplen < 4 * this_iphdr->ip_hl + sizeof(struct tcphdr)) {
    nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_HDR, this_iphdr,
		       this_tcphdr);//指示长度与实际不相符
    return;
  } // ktos sie bawi
  ...
  if (!nids_params.n_tcp_streams) return;//n_tcp_streams表示存放tcp_stream的哈希表大小
...
//ECN
#endif
  //经过上述检测，初步判断tcp包正常,下面判断数据包处于何种状态
  ...
  //有数据存在或者序列号不等于确认号并且序列号在窗口之外 已经确认过的序号,直接忽略
  if (
  ...
  //如果是rst包，关闭连接。将现有数据推送给注册的回调方，然后销毁这个流
